...
- **Club Onboarding Wizard v1.2 — Onboarding Completion Pack:** the wizard now finishes the migration story with a calm batch-detail drawer (`GET /api/onboarding/batches/:id`), per-step history (`GET /api/onboarding/history?step=<key>`), an honest replay/retry hint on every recorded batch, a "Try again with a corrected file" affordance whenever a step needs attention, an explicit ready/almost-ready/needs-attention completion panel on the go-live step, a short recommended-actions list, and a five-item first-30-days companion strip. No fake undo, no double-import — re-running a step is safe by design and the wizard reminds the user calmly. See [docs/onboarding.md](docs/onboarding.md).
...
- [docs/onboarding.md](docs/onboarding.md) — Club Onboarding Wizard + Import Templates (guided club setup, per-step templated imports, dependency-aware go-live, Go-Live Confidence Pack v1.1: history, last-import per step, readiness signals; Onboarding Completion Pack v1.2: batch-detail drawer, per-step history, calm replay/retry, completion panel, recommended actions, first-30-days companion)
